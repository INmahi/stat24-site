...
export function SiteNav() {
  ...
  return (
    <motion.div className="sticky top-0 z-30">
      <header className="relative overflow-hidden border-b border-transparent">
        <motion.div
          aria-hidden
          animate={scrolled ? "on" : "off"}
          className="absolute inset-0 border-b border-border/70 bg-background/95 backdrop-blur-md supports-backdrop-filter:bg-background/85"
          initial={false}
          transition={{ duration: 0.22, ease: "easeOut" }}
          variants={{
            off: { opacity: 0, boxShadow: "0 0 0 rgba(0, 0, 0, 0)" },
            on: { opacity: 1, boxShadow: "0 8px 24px rgba(0, 0, 0, 0.12)" },
          }}
        />
        <div
          className="absolute inset-x-0 bottom-0 h-px bg-border/70 transition-opacity duration-200"
